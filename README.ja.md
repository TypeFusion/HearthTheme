...
現在は Open VSX 互換エディタと VS Code で利用でき、4つのバリアントでセマンティクス階層を一貫して維持します。
...
| Web テーマ面 | Planned | 同じカラー言語をサイト/UI テーマへ拡張 |
| 他エディタ展開 | Planned | ツール横断でセマンティクス整合を維持 |
...
メンテナー向け主要ディレクトリ: `themes/`, `extension/`, `src/`, `scripts/`。
