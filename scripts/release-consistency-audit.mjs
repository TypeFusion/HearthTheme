...
const EXTENSION_PKG = path.join(ROOT, EXTENSION_PACKAGE_JSON_PATH)
const EXTENSION_CHANGELOG = path.join(ROOT, EXTENSION_CHANGELOG_PATH)
const SITE_PRODUCT_DATA = path.join(ROOT, SITE_PRODUCT_DATA_PATH)
const SITE_PRODUCT_IMPORT_FILES = [
  path.join(ROOT, SITE_SRC_DIR, 'layouts', 'Layout.astro'),
  path.join(ROOT, SITE_SRC_DIR, 'components', 'ui', 'HeroSection.astro'),
  path.join(ROOT, SITE_SRC_DIR, 'components', 'ui', 'FinalCtaSection.astro'),
  path.join(ROOT, SITE_SRC_DIR, 'components', 'ui', 'BaselineDocs.astro'),
  path.join(ROOT, SITE_SRC_DIR, 'components', 'ui', 'ColorSystem.astro'),
]
...
function parseChangelogRows(markdown) {
  ...
  for (const section of sections) {
    ...
    if (bullets.length === 0) {
      throw new Error(`packages/extension/CHANGELOG.md release ${version} is missing bullet entries.`)
    }
  ...
  if (rows.length === 0) {
    throw new Error('packages/extension/CHANGELOG.md does not contain any parseable release sections.')
  }
...
try {
  ...
  if (!pkg.version) {
    findings.push('packages/extension/package.json is missing `version`.')
  }
  ...
  if (releaseVersion && pkg.version && releaseVersion !== pkg.version) {
    findings.push(
      `release version mismatch: ${RELEASE_METADATA_PATH}=${releaseVersion}, packages/extension/package.json=${pkg.version}.`,
    )
  }

  if (!changelogVersion) {
    findings.push('packages/extension/CHANGELOG.md is missing a `## X.Y.Z` release heading.')
  } else if (releaseVersion && releaseVersion !== changelogVersion) {
    findings.push(
      `release version mismatch: ${RELEASE_METADATA_PATH}=${releaseVersion}, packages/extension/CHANGELOG.md top=${changelogVersion}.`,
    )
  }

  if (topSection && /update notes pending/i.test(topSection)) {
    findings.push(
      'packages/extension/CHANGELOG.md top release still contains placeholder text (`Update notes pending`).',
    )
  }
  ...
  for (const [field, expectedValue] of Object.entries(expectedPkg)) {
    ...
    if (!sameJson(actualValue, expectedValue)) {
      addMismatch(`packages/extension/package.json ${field}`, actualValue, expectedValue)
    }
  }

  if (!siteProductSource.includes('Auto-generated by scripts/generate-site-assets.mjs')) {
    findings.push('packages/site/src/data/product.ts is missing the generated header.')
  }
  ...
  for (const expectedValue of requiredSiteProductValues) {
    if (!siteProductSource.includes(expectedValue)) {
      findings.push(`packages/site/src/data/product.ts is missing expected value: ${expectedValue}.`)
    }
  }
  ...
  for (const file of SITE_PRODUCT_IMPORT_FILES) {
    ...
    if (!source.includes('data/product')) {
      findings.push(`${path.relative(ROOT, file)} should import shared product metadata from packages/site/src/data/product.ts.`)
    }
